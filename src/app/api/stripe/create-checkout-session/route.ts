...
export async function POST(req: Request) {
  ...
  // Check if productId is missing or empty
  if (!subAccountConnectAccId || !prices.length || prices.some(price => !price.productId)) {
    return new NextResponse("Stripe Account Id or priceId is missing or invalid", {
      status: 400,
    });
  }
    ...
    const session = await stripe.checkout.sessions.create(
      {
        line_items: prices.map((price) => ({
          price: price.productId, // Ensure valid productId here
          quantity: 1,
        })),
